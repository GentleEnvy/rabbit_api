...
class AvoidInbreedingService:
    ...
    def validate_rabbit(self, rabbit):
        ...
        if rabbit.current_type == Rabbit.TYPE_BUNNY or \
           rabbit.cast.manager.age.days < self.days_for_ready_mating:
            raise ValueError('This rabbit is too young to breed')
    
    def sort_rabbits_by_output_efficiency(self, rabbits: QuerySet) -> list:
        sorted_rabbits = [
            rabbit for rabbit in sorted(rabbits.all(),
                key=lambda rabbit: rabbit.cast.manager.output_efficiency)
        ]
            ...
            rnd = random.choice(
                [
                    FatherRabbit(
                        ...
                        is_male=True,
                        is_ill=random.choices([True, False], weights=[0.05, 0.95], k=1)[
                            0],
                        current_type='P',
                        cage_id=cage.id
                    ),
